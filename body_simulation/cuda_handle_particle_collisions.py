...
extern "C" __global__ void handle_collisions_kernel(
    double *positions, double *velocities, double *masses, double *sizes,
    int N, double elasticity) {
    ...
    double pos_i[2], vel_i[2], mass_i, size_i;
    pos_i[0] = positions[idx * 2];
    pos_i[1] = positions[idx * 2 + 1];
    vel_i[0] = velocities[idx * 2];
    vel_i[1] = velocities[idx * 2 + 1];
    mass_i = masses[idx];
    size_i = sizes[idx];
    ...
    for (int tile = 0; tile < (N + blockDim.x - 1) / blockDim.x; ++tile) {
        ...
        if (tile_idx < N) {
            ...
            shared_positions[threadIdx.x * 2 + 1] = positions[tile_idx * 2 + 1];
            shared_velocities[threadIdx.x * 2] = velocities[tile_idx * 2];
            shared_velocities[threadIdx.x * 2 + 1] = velocities[tile_idx * 2 + 1];
        }
        ...
        for (int j = 0; j < blockDim.x; ++j) {
            ...
            // Check for collision
            double combined_radius = size_i + sizes[other_idx];
            if (distance < combined_radius && distance > 0) {
                double normal_x = delta_x / distance;
                double normal_y = delta_y / distance;

                double rel_vel_x = vel_i[0] - shared_velocities[j * 2];
                double rel_vel_y = vel_i[1] - shared_velocities[j * 2 + 1];

                double vel_normal = rel_vel_x * normal_x + rel_vel_y * normal_y;

                if (vel_normal < 0) {
                    double overlap = max(combined_radius - distance, 0.0);
                    double max_overlap_correction = 0.1 * combined_radius;
...
def handle_particle_collisions_cudaKernel(positions, velocities, masses, sizes, elasticity):
    """
    ...
    Args:
        ...
        velocities: CuPy array of shape (N, 2) containing particle velocities
        masses: CuPy array of shape (N, 1) containing particle masses
        sizes: CuPy array of shape (N, 1) containing particle sizes
    ...
    sizes = sizes.astype(cp.float64).reshape(-1)
    ...
    collision_kernel(
        (blocks_per_grid,),
        (threads_per_block,),
        (positions.ravel(), velocities.ravel(), masses, sizes, N, cp.float64(elasticity))
    )
