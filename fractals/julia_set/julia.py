...
def run_cpu(size_multi=2, max_iters=50, show=True, save=False):
    """Run the Julia fractal on the CPU."""
    ...
    if show or save:
        ...
        plt.title(f"Julia Set (CPU, iters={max_iters})")
        plt.tight_layout()
    if show:
        plt.show()
    if save:
        plt.imsave(f"fractals/julia_set/plots/cpu_julia_size{size_multi}_iters{max_iters}.png", image, dpi=3000)
    return image

def run_gpu_cuda(size_multi=2, max_iters=50, show=True, save=False):
    """Run the Julia fractal on the GPU using Numba."""
    width = 750 * size_multi
    height = 500 * size_multi
    ...
    if show or save:
        plt.imshow(image, cmap='seismic')
        plt.title(f"Julia Set (GPU, iters={max_iters})")
        plt.tight_layout()
    if show:
        plt.show()
    if save:
        plt.imsave(f"fractals/julia_set/plots/cuda_julia_size{size_multi}_iters{max_iters}.png", image, dpi=3000)
    return image


def run_gpu_opencl(size_multi=2, max_iters=50, show=True, save=False):
    """Run the Julia fractal on the GPU using PyOpenCL."""
    width = 750 * size_multi
    height = 500 * size_multi
    ...
    if show or save:
        plt.imshow(image, cmap='seismic')
        plt.title(f"Julia Set (GPU with PyOpenCL, iters={max_iters})")
        plt.tight_layout()
    if show:
        plt.show()
    if save:
        plt.imsave(f"fractals/julia_set/plots/opencl_julia_size{size_multi}_iters{max_iters}.png", image, dpi=3000)
    return image


def run_gpu_cupy(size_multi, max_iters, show=True, save=False):
    """Run the Julia fractal on the GPU using CuPy."""
    width = 750 * size_multi
    height = 500 * size_multi
    ...
    if show or save:
        plt.imshow(image, cmap='seismic')
        plt.title(f"Julia Set (GPU with CuPy, iters={max_iters})")
        plt.tight_layout()
    if show:
        plt.show()
    if save:
        plt.imsave(f"fractals/julia_set/plots/cupy_julia_size{size_multi}_iters{max_iters}.png", image, dpi=3000)
    return image
...
def main():
    ...
    print(f"Creating Julia Set of size {750*SIZE_MULTI}x{500*SIZE_MULTI} with {MAX_ITERS} iterations")
    ...
    # Runs single and shows the Julia fractal 
    #--------------------------------------------------------------------------------------------
    if CPU:
        print("\nRunning CPU version...")
