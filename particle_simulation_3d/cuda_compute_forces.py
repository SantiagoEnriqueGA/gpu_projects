...
void compute_forces_kernel(const double* positions,
    ...
    if (i < N) {
        ...
        double fy = 0.0;
        double fz = 0.0;
        double pos_i_x = positions[i * 3];
        double pos_i_y = positions[i * 3 + 1];
        double pos_i_z = positions[i * 3 + 2];
        double mass_i = masses[i];
        
        // Compute forces from all other particles
        for (int j = 0; j < N; j++) {
            if (i != j) {
                double dx = -(positions[j * 3] - pos_i_x);
                double dy = -(positions[j * 3 + 1] - pos_i_y);
                double dz = -(positions[j * 3 + 2] - pos_i_z);
                
                double dist_squared = dx * dx + dy * dy + dz * dz + epsilon;
...
def compute_forces_cudaKernel(positions, masses, G=1.0, epsilon=1e-5):
    ...
    Returns:
        CuPy array of shape (N, 3) containing the forces on each particle
    """
    N = positions.shape[0]
