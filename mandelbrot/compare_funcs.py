...
print("Generating Mandelbrot fractal (CPU)...")
image = np.zeros((height, width), dtype=np.uint8)
create_fractal_cpu(-2.0, 1.0, -1.0, 1.0, image, MAX_ITERS)
cpu_image = image.copy()

if check_numba_cuda():
    print("\nGenerating Mandelbrot fractal (GPU Numba CUDA)...")
    image = np.zeros((height, width), dtype=np.uint8)
...
if check_openCl():
    print("\nGenerating Mandelbrot fractal (GPU OpenCL)...")
    image = np.zeros((height, width), dtype=np.uint8)
...
if check_cupy():
    print("\nGenerating Mandelbrot fractal (GPU CuPy)...")
    image = np.zeros((height, width), dtype=np.uint8)
